...
with st.sidebar:
    ...
    model = st.text_input(
        "Model",
        value=DEFAULT_MODEL,
        help="Current free-tier model IDs: gemini-3-flash (fast), "
        "gemini-3.5-flash (newer), gemini-3.1-pro (strongest, tighter quota). "
        "If you get a 404 NOT_FOUND error, use the detector below or type a "
        "current ID from https://aistudio.google.com — the app also auto-switches "
        "to an available model when it can.",
    )
    with st.expander("🔍 Detect models my key supports"):
        st.caption(
            "Google retires/renames model IDs. This asks your API key exactly "
            "which model IDs work (Google's ListModels API)."
        )
    ...
    if generate_clicked:
        ...
        try:
            ...
            if "NOT_FOUND" in msg or "no longer available" in msg.lower() or "404" in msg:
                st.error(
                    "That Gemini model ID was renamed or retired by Google. "
                    "In the sidebar, set **Model** to a current ID — try "
                    "`gemini-3.5-flash`, `gemini-3-flash` or `gemini-3.1-pro` — "
                    "then run again.\n\n" + msg
                )
                ...
                st.error(
                    ...
                    "or switch to another model in the sidebar (e.g. "
                    "`gemini-3.5-flash`).\n\n" + msg
                )
            else:
                st.error(msg)
