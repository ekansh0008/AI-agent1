...
from src.neg_pipeline import run_playbook_pipeline, run_practice_pipeline
...
if mode.startswith("🤝"):
    ...
    # ------------------------------- Playbook ------------------------------
    with tab_play:
        st.header("⚡ Negotiation Playbook")
        st.write(
            "Paste the scenario brief the moment you receive it. The agent analyses "
            "the dispute, researches the real-world facts, and builds your game plan: "
            "objectives, BATNA, concession ladder, coalitions, argument bank with "
            "sources, crisis contingencies and a one-page card. For a faster "
            "round-day run pick **Quick draft** depth in the sidebar."
        )
        with st.form("playbook_form"):
            brief = st.text_area(
                "Scenario brief (paste full text) — or just a one-line scenario *",
            ...
            with c1:
                ...
                parties = st.text_input("Other parties (comma-separated, optional)", key="pb_others")
            ...
            with c2:
                team = st.text_input("Team members", key="pb_team", placeholder="You + your partner")
                institution = st.text_input("Institution", key="pb_inst")
                region = st.text_input("Region context", key="pb_region", value="South Asia")
                sccenario_title = st.text_input("Scenario title (optional — agent will infer)", key="pb_title")
            go = st.form_submit_button("🧭 Research & build playbook", use_container_width=True)
        ...
        if go:
            if not brief.strip():
                ...
                meta = NegoMeta(
                    scenario_title=sccenario_title,
                ...
                status_box = st.status("Building your playbook…", expanded=True)
                result = None
                try:
                    for event_ in run_playbook_pipeline(meta, api_key.strip(), model.strip(), depth):
        ...
        if neg_result is not None:
            st.divider()
            _show_document(
                f"📋 {neg_result.title} — Playbook", neg_result.markdown,
                neg_result.docx_path, neg_result.md_path, neg_result.sources,
                neg_result.queries, neg_result.analysis, "playbook",
            )

    # ------------------------------- Practice ------------------------------
    with tab_prac:
        st.header("🎲 Practice Match Generator")
        st.write(
            "Rehearse with your teammate before the tournament. Pick a category and "
            "the difficulty of your upcoming round — you'll get a shared match brief, "
            "two **confidential** team briefs (hidden objectives, pressure points), "
            "and a judge's note with the plausible agreement zone + scoring rubric."
        )
        with st.form("practice_form"):
            ...
            with c2:
                pr_research = st.toggle(
                    "Ground in real-world facts (slower, more realistic)", value=False, key="pr_research"
                )
            go_prac = st.form_submit_button("🎲 Generate practice match", use_container_width=True)

        if go_prac:
            if not api_key:
                st.error("Add your Gemini API key in the sidebar (free: https://aistudio.google.com/apikey).")
            else:
                status_box = st.status("Generating your practice match…", expanded=True)
                result = None
                try:
                    for event_ in run_practice_pipeline(
                        category, difficulty, pr_region, pr_research, api_key.strip(), model.strip()
                    ):
                        if event_["stage"] == "error":
                            status_box.update(label="Something went wrong", state="error")
                            st.error(event_["text"])
        ...
        if practice_result is not None:
            st.divider()
            st.info(
                "🔒 Print the pack and hand each team only their confidential brief — "
                "peek at the judge's note only after the match."
            )
            _show_document(
                f"🎲 {practice_result.title} — Practice match pack",
                practice_result.markdown, practice_result.docx_path,
                practice_result.md_path, practice_result.sources, None,
                practice_result.analysis, "practice",
            )
