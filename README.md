...
**⚡ Playbook builder** — paste the round's scenario brief (the confidential brief is
released only at round start, so paste it the moment you get it). The agent:

```
scenario brief
   │
   ▼
1. DECODE ──► title, issue areas, parties, stakes, real-world analog treaties
   ▼
2. DEEP RESEARCH ──► hard data, treaty provisions, past negotiation outcomes,
   │                  party interests (UN/treaty texts via keyless web search)
   ▼
3. PLAYBOOK ──► situation brief ▸ stakeholder map ▸ ranked objectives +
   │            BATNA & red lines ▸ strategy + concession ladder ▸ coalition plan
   │            ▸ argument bank (cited) ▸ crisis contingencies ▸ draft communiqué
   │            skeleton ▸ diplomatic phrasebook ▸ one-page quick-reference card
   ▼
4. EXPORT ──► formatted .docx + .md
```

Built for what judges reward: consensus-building, realistic implementable deals,
adaptability, professional diplomacy — not stubbornly defending a position.

**🎲 Practice match generator** — rehearse with your 2-person team: pick a
category (cross-border resources / transnational security / crisis management /
global summit) and a difficulty matching your round (Preliminary → Grand Final);
you get a shared match brief, **two confidential team briefs** (hidden objectives
and pressure points) and a judge's note with the plausible agreement zone + a
scoring rubric matched to the competition's values.
